...
void* wxWebViewChromium::sandbox_info = NULL;
           ...
           const wxString& name)
{
	...
	CefBrowserHost::CreateBrowser(info, m_clientHandler.get(),
...
void wxWebViewChromium::OnSize(wxSizeEvent& event)
{
    ...
    //{
		...
		HWND handle = FindWindowEx(GetHWND(), NULL, _("CefBrowserWindow"), NULL);
...
bool wxWebViewChromium::StartUp(int &code, const wxString &path)
{
    ...
    if(path == "")
    {
		code = CefExecuteProcess(args, CefRefPtr<CefApp>(), sandbox_info);
        if(code >= 0)
            return false;
    }
	...
	return CefInitialize(args, settings, CefRefPtr<CefApp>(), sandbox_info);
}
...
int wxWebViewChromium::StartUpSubprocess()
{
	...
	return CefExecuteProcess(args, CefRefPtr<CefApp>(), sandbox_info);
}
